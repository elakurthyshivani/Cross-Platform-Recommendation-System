...
if($_SERVER["REQUEST_METHOD"]=="POST") {
    if(isset($_POST['delete']))  {
        $arr=explode(", ", $_POST['delete']); // (User ID, Content ID)
        removeRating($arr[0], $arr[1]);
    }
}
        ...
        <main class="main-2 col-12"> 
            <div class="full-page-content col-12">
                ...
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                    ...
                    ?>
                        <tr>
                            <td class="user-id"><?php echo $rating["userID"]; ?></td>
                            <td class="show-title"><?php echo $rating["title"]; ?></td>
                            <td class="rating"><?php echo $rating["rating"]; ?></td>
                            <td class="rated-at-time"><?php echo $rating["ratedAt"]; ?></td>
                            <td class="delete-link">
                                <button type="submit" name="delete" 
                                        value="<?php echo $rating["userID"].', '.$rating["contentID"]; ?>">
                                    <a>Delete</a>
                                </button>
                            </td>
                        </tr>
